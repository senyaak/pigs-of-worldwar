...
//
// A swimming pig hangs SWIM_SINK below the surface, and the frame the
// per-texel water test concedes it is swimming, it drops that whole distance
// at once — on this shore the seabed sits exactly AT the water level, so
// nothing about the ground eases it in. The camera used to follow, which
// read as a lurch. It frames the pig less that sink now, so the two moves
// cancel: the pig sinks, the view does not move.
...
/** Dry, walkable ground on CAMP with open water straight north of it. */
function shoreOnCamp(): { x: number; z: number; query: TerrainQuery } {
  ...
  throw new Error('no shore with open water north of it on CAMP')
}
...
test('the camera holds a swimming pig up by its whole sink', async ({ app }) => {
  ...
  // Then swim out. Held down long enough to cross the shelf, land, and have
  // the per-texel test concede the pig is swimming.
