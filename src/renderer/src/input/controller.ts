...
export interface DebugHooks {
  ...
  /** The sky dome: which mood loaded, how big it is, how far its centre has
   * drifted off the camera — which is meant to be nothing — and the haze that
   * came with the mood (three/sky.ts). Null on a map whose dome would not
   * load. */
  sky(): {
    mood: string
    triangles: number
