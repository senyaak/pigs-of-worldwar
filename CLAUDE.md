...
- `src/lib/game/` — the rules (`Game`, `TerrainQuery`). Pure too, so the
  domain specs drive them directly.
...
slope-sliding. Next up is weapons — and footstep audio, whose event source is
already settled in `../pigs-disasm/anim/audio-events.md`.
